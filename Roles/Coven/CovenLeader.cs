...
internal class CovenLeader : CovenManager
{
    ...
    {
        return target.Is(CustomRoles.Enchanted) || (target.Is(CustomRoles.Romantic) && Romantic.BetPlayer.TryGetValue(target.PlayerId, out var romanticTarget) && romanticTarget == covenLeader.PlayerId) || (target.Is(CustomRoles.Lawyer) && Lawyer.TargetList.Contains(covenLeader.PlayerId)) || (target.Is(CustomRoles.Medic) && Medic.IsProtected(covenLeader.PlayerId)) || (target.Is(CustomRoles.Lovers) && covenLeader.Is(CustomRoles.Lovers)) || (target.Is(CustomRoles.Monarch) && covenLeader.Is(CustomRoles.Knighted)) || (target.Is(CustomRoles.Follower) && Follower.BetPlayer.TryGetValue(target.PlayerId, out var followerTarget) && followerTarget == covenLeader.PlayerId) || (target.Is(CustomRoles.SchrodingersCat) || SchrodingersCat.teammate.TryGetValue(target.PlayerId, out var catTeammate) && catTeammate == covenLeader.PlayerId) || (target.Is(CustomRoles.Crusader) && (target.GetRoleClass() is Crusader crus && crus.ForCrusade.Contains(covenLeader.PlayerId))) ||
